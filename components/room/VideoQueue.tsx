    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-white">Video Queue</h2>
            <p className="text-gray-400 text-xs sm:text-sm lg:text-base">
              Add songs and manage the karaoke queue
            </p>
          </div>
        </div>
        <Badge variant="secondary" className="bg-red-500/20 text-red-400 px-3 py-1 text-xs sm:text-sm">
      ...
      <Card className="bg-gray-800/50 border-red-500/30 overflow-hidden">
        <Tabs defaultValue="add" className="w-full">
          <div className="border-b border-gray-700 bg-gray-900/50">
            <TabsList className="grid w-full grid-cols-3 bg-transparent border-none h-12 sm:h-14">
              <TabsTrigger
                value="add"
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <Button
                    ...
                    className="bg-red-500 hover:bg-red-600 px-4 sm:px-6 h-10 sm:h-12 w-full sm:w-auto"
                  >
                    {isAdding ? (
                    ...
                    <div className="max-h-64 sm:max-h-80 overflow-y-auto">
                      <div className="space-y-3">
                        {searchResults.map((video) => (
                          <div
                            key={video.id}
                            className="flex items-center gap-2 sm:gap-4 p-3 sm:p-4 bg-gray-700/30 rounded-lg hover:bg-gray-700/50 transition-all group"
                          >
                            <Image
                              src={video.thumbnail}
                              alt={video.title}
                            ...
                            <Button
                              ...
                              className="bg-red-500 hover:bg-red-600 flex-shrink-0 h-8 w-8 sm:h-auto sm:w-auto sm:px-3"
                            >
                ...
                {isLoadingPopular ? (
                  <div className="flex items-center justify-center py-8 sm:py-12">
                    <div className="text-center">
                      <Loader2 className="w-6 h-6 sm:w-8 sm:h-8 animate-spin text-red-500 mx-auto mb-3" />
                      <span className="text-gray-400 text-sm">Loading popular songs...</span>
                    </div>
                  </div>
                ) : popularSongs.length > 0 ? (
                  <div className="max-h-64 sm:max-h-96 overflow-y-auto overflow-x-hidden w-[85vw]">
                    <div className="space-y-3 pr-1">
                      {popularSongs.map((video) => (
                        <div
                          key={video.id}
                          className="flex items-center gap-2 sm:gap-4 p-3 sm:p-4 bg-gray-700/30 rounded-lg hover:bg-gray-700/50 transition-all group w-full"
                        >
                          <Image
                            src={video.thumbnail}
                            alt={video.title}
                          ...
                          <div className="flex-1 min-w-0 overflow-hidden">
                            ...
                            <p className="text-[10px] sm:text-xs text-gray-400 truncate mt-1">{video.channelTitle}</p>
                            <div className="flex items-center gap-2 sm:gap-3 mt-2 flex-wrap">
                              <Badge variant="secondary" className="text-[10px] sm:text-xs bg-gray-600 text-gray-300 flex-shrink-0">
                                {video.duration}
                              </Badge>
                              {isPopularMock && (
                                <Badge variant="secondary" className="text-[10px] sm:text-xs bg-yellow-500/20 text-yellow-400 flex-shrink-0">
                                  Mock
                                </Badge>
                              )}
                              <button
                                onClick={() => openVideoInNewTab(video.id)}
                                className="text-[10px] sm:text-xs text-blue-400 hover:text-blue-300 flex items-center gap-1 transition-colors flex-shrink-0"
                              >
                                <ExternalLink className="w-2 h-2 sm:w-3 sm:h-3" />
                                Preview
                              </button>
                  ...
                  <div className="text-center py-8 sm:py-12 text-gray-400">
                    <Music className="w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-3 opacity-50" />
                        ...
                        >
                          ...
                          <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
                            <Button
                              ...
                              size="sm"
                              variant="ghost"
                              className="text-gray-400 hover:text-white h-6 w-6 sm:h-8 sm:w-8 p-0"
                            >
                            ...
                            {isHost && (
                              <Button
                                ...
                                size="sm"
                                variant="ghost"
                                className="text-red-400 hover:text-red-300 hover:bg-red-500/20 h-6 w-6 sm:h-8 sm:w-8 p-0"
                              >
